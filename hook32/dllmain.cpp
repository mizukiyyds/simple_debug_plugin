...
void hook_ReadWriteProcessMemory()
{
    ...
    DWORD old_protect;
    VirtualProtect(address_read,5, PAGE_EXECUTE_READWRITE, &old_protect);
    ((BYTE*)address_read)[0]=0xE9;
    *(DWORD*)((DWORD)address_read+1)=(DWORD)hook_read_func-(DWORD)address_read-5;
    VirtualProtect(address_read,5, old_protect, &old_protect);

    VirtualProtect(address_write,5, PAGE_EXECUTE_READWRITE, &old_protect);
    ((BYTE*)address_write)[0]=0xE9;
    *(DWORD*)((DWORD)address_write+1)=(DWORD)hook_write_func-(DWORD)address_write-5;
    VirtualProtect(address_write,5, old_protect, &old_protect);
...
{
    DWORD old_protect;
    VirtualProtect(address_open,5, PAGE_EXECUTE_READWRITE, &old_protect);
    ((BYTE*)address_open)[0]=0xE9;
    *(DWORD*)((DWORD)address_open+1)=(DWORD)hook_open_func-(DWORD)address_open-5;
    VirtualProtect(address_open,5, old_protect, &old_protect);
}
